...
def clusterings(numbofclousters):
    if resturantValidator():
        ...
        while i <= itirations:
            ...
            for clusers in clusters:
                clusers.containingCases = []

            for restant in resturants:
                putinindex = compareson(restant, clusters)

                clusters[putinindex].containingCases.append(restant)
...
def AVGCenteroid(containingCases):
    ...
    for row in containingCases:
        ...
        if row.PriceClass == 'High':
            ...
            Priceclassi += 1
        tempdistans = tempdistans + float(row.DistansUni)
...
def compareson(compareInput, cluster):
    ...
    for centeroidInfo in centeroid:                                                                                     # Comparing centeroid value to all resturants in database one by one
        for sub_Centeroid_Info in centeroidInfo:
            ...
            returningResults = float((returningResults / 5) * 3)

            if returningResults >= 0 and returningResults < 1.5:
                returnScore.append(0)                                                                                   # Store away the compared values
            if returningResults >= 1.5 and returningResults < 2.5:
                returnScore.append(1)
            if returningResults >= 2.5 and returningResults < 3.5:
            ...
            for e in returnScore:                                                                                       # Takes out the higest comparison values and the index of that value and wich cluster it belong to.
                indexing += 1
                if e > temp_higest_numb:
                    temp_higest_numb = e
